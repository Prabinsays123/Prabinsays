...
(function scrollReveals() {
  const targets = document.querySelectorAll(".section h2, .project-grid, .skill-web");
...
/* =========================================================
   WEB CRAWL GAME (Snake, reskinned)
   ========================================================= */
(function webCrawlGame() {
  ...
  let spiderBody, direction, nextDirection, fly, score, loopId, speedMs;
  const BEST_KEY = "prabinsays-webcrawl-best";
  ...
  function placeFly() {
    ...
    } while (spiderBody.some(segment => segment.x === position.x && segment.y === position.y));
    fly = position;
  }
  ...
  function draw() {
    ...
    ctx.strokeStyle = "rgba(124, 154, 184, 0.25)";
    ctx.lineWidth = 1;
    spiderBody.forEach((segment, index) => {
      if (index === 0) return;
      const prev = spiderBody[index - 1];
      ctx.beginPath();
      ctx.moveTo(prev.x * CELL + CELL / 2, prev.y * CELL + CELL / 2);
      ctx.lineTo(segment.x * CELL + CELL / 2, segment.y * CELL + CELL / 2);
      ctx.stroke();
    });

    ctx.fillStyle = COLORS.fly;
    ctx.beginPath();
    ctx.arc(fly.x * CELL + CELL / 2, fly.y * CELL + CELL / 2, CELL * 0.22, 0, Math.PI * 2);
    ctx.fill();

    spiderBody.forEach((segment, index) => {
      ctx.fillStyle = index === 0 ? COLORS.spiderHead : COLORS.spiderBody;
      const pad = 3;
      ctx.beginPath();
      ctx.roundRect(
        segment.x * CELL + pad,
  ...
  function step() {
    ...
    const head = {
      x: spiderBody[0].x + direction.x,
      y: spiderBody[0].y + direction.y
    };

    const hitsWall = head.x < 0 || head.y < 0 || head.x >= GRID_SIZE || head.y >= GRID_SIZE;
    const hitsSelf = spiderBody.some(segment => segment.x === head.x && segment.y === head.y);
  ...
  function endGame() {
    ...
    if (score > best) {
      ...
      overlayTitle.textContent = "Web broken";
...
/* =========================================================
   SKILL WEB THREAD ANIMATION TRIGGER
   ========================================================= */
(function skillWebTrigger() {
  const el = document.getElementById("skill-web");
  if (!el) return;
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
